            ...
            <div className="flex items-center space-x-4">
                ...
                <Link href="/dashboard">
                  ...
                  Back to Dashboard
                             ...
                             <h1 className="text-xl font-bold text-[var(--foreground)]">Practice Problems</h1>
            </div>
          </div>
        </div>
      </header>
            ...
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-[var(--muted-foreground)]" />
                   ...
                   className="pl-10 border-[var(--border)] focus:border-[var(--ring)]"
                   style={{ color: 'var(--foreground, #0a0903)' }}
                 />
              </div>
            </div>
                         <Select>
               <SelectTrigger className="w-full sm:w-48 border-[var(--border)] focus:border-[var(--ring)]" style={{ color: 'var(--foreground, #0a0903)' }}>
              ...
              <SelectContent>
                ...
                <SelectItem value="advanced">Advanced</SelectItem>
                         ...
                         <Select>
               <SelectTrigger className="w-full sm:w-48 border-[var(--border)] focus:border-[var(--ring)]">
            ...
            <Card key={problem.id} className="card-ut hover-lift group">
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center space-x-1 text-sm text-[var(--muted-foreground)]">
                    <Clock className="h-3 w-3" />
                    <span>{problem.time_limit || '5'} min</span>
                  </div>
                </div>
                                 <CardTitle className="text-lg text-[var(--foreground)] group-hover:text-[var(--primary)] transition-colors line-clamp-2">
                  {problem.title}
                </CardTitle>
                <CardDescription className="text-[var(--muted-foreground)] line-clamp-2">
                  {problem.description}
                </CardDescription>
              </CardHeader>
        ...
        {(!problems || problems.length === 0) && (
          <div className="text-center py-12">
            <div className="w-16 h-16 bg-[var(--muted)] rounded-full flex items-center justify-center mx-auto mb-4">
              <Code className="h-8 w-8 text-[var(--muted-foreground)]" />
            </div>
                         <h3 className="text-lg font-medium text-[var(--foreground)] mb-2">No Problems Available</h3>
